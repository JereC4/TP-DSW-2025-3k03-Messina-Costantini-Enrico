...
/** Cliente de Supabase con la service role key; null si no está configurado (ver .env.example). */
function getClient(): SupabaseClient | null {
  if (client !== undefined) return client;
  client = env.SUPABASE_URL && env.SUPABASE_SERVICE_ROLE_KEY
...
export async function uploadAvatar(userId: bigint, buffer: Buffer, contentType: string, extension: string): Promise<string> {
  ...
  const { error } = await supabase.storage.from(env.SUPABASE_AVATARS_BUCKET).upload(path, buffer, { contentType, upsert: false });
  if (error) throw { status: 502, code: "STORAGE_UPLOAD_FAILED", message: `No se pudo subir la foto: ${error.message}` };

  const { data } = supabase.storage.from(env.SUPABASE_AVATARS_BUCKET).getPublicUrl(path);
...
export async function deleteAvatarByUrl(url: string | null | undefined): Promise<void> {
  ...
  const marker = `${PUBLIC_URL_MARKER}${env.SUPABASE_AVATARS_BUCKET}/`;
  const idx = url.indexOf(marker);
  if (idx === -1) return;
  const path = url.slice(idx + marker.length);
  try {
    await supabase.storage.from(env.SUPABASE_AVATARS_BUCKET).remove([path]);
  } catch {
